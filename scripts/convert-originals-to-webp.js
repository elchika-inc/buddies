...
async function updateDatabase(results) {
  ...
  // ペットタイプごとにグループ化
  const catIds = successfulConversions
    .filter(c => c.pet_type === 'cats')
    .map(c => c.pet_id);
  
  const dogIds = successfulConversions
    .filter(c => c.pet_type === 'dogs')
    .map(c => c.pet_id);
...
async function main() {
  ...
  if (catsToConvert.length > 0) {
    ...
    for (let i = 0; i < catsToConvert.length; i += BATCH_SIZE) {
      ...
      const batchResults = await Promise.all(
        batch.map(img => convertToWebp(img.Key, 'cats'))
      );
  ...
  if (dogsToConvert.length > 0) {
    ...
    for (let i = 0; i < dogsToConvert.length; i += BATCH_SIZE) {
      ...
      const batchResults = await Promise.all(
        batch.map(img => convertToWebp(img.Key, 'dogs'))
      );
  ...
  if (successful > 0) {
    ...
    const avgSavings = results
      .filter(r => r && r.success)
      .reduce((sum, r) => sum + r.savings, 0) / successful;
    ...
    console.log(`  💰 Total saved: ${((totalSourceSize - totalWebpSize) / 1024 / 1024).toFixed(2)}MB`);
  }
