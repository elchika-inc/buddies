...
function detectInconsistencies(r2Images, dbPets) {
  ...
  // 詳細表示
  const missingInDB = inconsistencies.filter(i => 
    (i.r2.hasJpeg && !i.db.hasJpeg) || (i.r2.hasWebp && !i.db.hasWebp)
  );
  const extraInDB = inconsistencies.filter(i => 
    (!i.r2.hasJpeg && i.db.hasJpeg) || (!i.r2.hasWebp && i.db.hasWebp)
  );
...
async function updateDatabase(inconsistencies) {
  if (CONFIG.DRY_RUN) {
    ...
    for (const item of inconsistencies.slice(0, 10)) {
      console.log(`  ${item.id}: JPEG ${item.db.hasJpeg}→${item.r2.hasJpeg}, WebP ${item.db.hasWebp}→${item.r2.hasWebp}`);
    }
  ...
  for (let i = 0; i < inconsistencies.length; i += batchSize) {
    ...
    const payload = {
      results: batch.map(item => ({
        ...
        pet_type: item.type,
        success: true,
        jpegUrl: item.r2.hasJpeg ? `https://${CONFIG.R2_BUCKET_NAME}.r2.dev/pets/${item.type}s/${item.id}/original.jpg` : null,
        webpUrl: item.r2.hasWebp ? `https://${CONFIG.R2_BUCKET_NAME}.r2.dev/pets/${item.type}s/${item.id}/optimized.webp` : null
      }))
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`  ✅ Batch ${Math.floor(i / batchSize) + 1}: Updated ${result.updatedCount || batch.length} pets`);
      } else {
        errorCount += batch.length;
        console.error(`  ❌ Batch ${Math.floor(i / batchSize) + 1}: Failed with status ${response.status}`);
      }
    } catch (error) {
      errorCount += batch.length;
...
async function generateReport(inconsistencies) {
  ...
  const report = {
    ...
    summary: {
      ...
      missingInDB: inconsistencies.filter(i => 
        (i.r2.hasJpeg && !i.db.hasJpeg) || (i.r2.hasWebp && !i.db.hasWebp)
      ).length,
      extraInDB: inconsistencies.filter(i => 
        (!i.r2.hasJpeg && i.db.hasJpeg) || (!i.r2.hasWebp && i.db.hasWebp)
      ).length
    },
...
if (!CONFIG.R2_ACCOUNT_ID || !CONFIG.R2_ACCESS_KEY_ID || !CONFIG.R2_SECRET_ACCESS_KEY) {
  ...
  console.error('\nExample usage:');
  console.error('  R2_ACCOUNT_ID=xxx R2_ACCESS_KEY_ID=xxx R2_SECRET_ACCESS_KEY=xxx node fix-image-db-sync.js --dry-run');
  process.exit(1);
}
