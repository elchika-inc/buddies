...
function identifyOrphanedImages(r2Data, dbPetIds) {
  ...
  console.log(`  ⚠️  Found ${orphaned.length} orphaned pet IDs with ${orphaned.reduce((sum, o) => sum + o.files.length, 0)} files`);
...
async function main() {
  ...
  try {
    ...
    // 総サイズ計算
    const totalSize = orphanedImages.reduce((sum, o) => 
      sum + o.files.reduce((s, f) => s + f.size, 0), 0
    );
