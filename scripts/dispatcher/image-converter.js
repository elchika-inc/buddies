...
async function main() {
  ...
  for (let i = 0; i < petsWithMode.length; i += BATCH_SIZE) {
    ...
    const batchResults = await Promise.all(
      batch.map(pet => convertImage(pet))
    );
    results.push(...batchResults);
    
    console.log(`  📊 Progress: ${Math.min(i + BATCH_SIZE, petsWithMode.length)}/${petsWithMode.length}`);
  }
  ...
  if (successful > 0) {
    ...
    if (jpegResults.length > 0 && webpResults.length > 0) {
      const totalJpegSize = jpegResults.reduce((sum, r) => sum + r.jpegSize, 0);
      const totalWebpSize = webpResults.reduce((sum, r) => sum + r.webpSize, 0);
      console.log(`  💾 Overall savings: ${(100 - (totalWebpSize / totalJpegSize * 100)).toFixed(1)}%`);
    }
  }
