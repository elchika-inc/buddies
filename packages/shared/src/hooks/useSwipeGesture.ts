...
export function useSwipeGesture(props: SwipeGestureProps): SwipeGestureState {
  ...
  const handleEnd = () => {
    ...
    let action: SwipeDirection | null = null
    ...
    if (isUpwardSwipe) {
      ...
      if (enableVerticalSwipe && dragOffset.y < -verticalThreshold && absY > absX * SWIPE_THRESHOLDS.RATIO.VERTICAL_DOMINANT) {
        action = 'superlike'
      }
      // 上向きだがSUPER LIKEの条件を満たさない場合は何もしない
    } else {
      // 下方向または水平：LIKE/PASSのみ判定
      if (absX > horizontalThreshold && absX > absY * SWIPE_THRESHOLDS.RATIO.HORIZONTAL_DOMINANT) {
