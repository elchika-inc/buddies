...
type GenericSwipeScreenProps<T extends BaseAnimal> = {
  onShowLiked: () => void
...
export function GenericSwipeScreen<T extends BaseAnimal>({ 
  onShowLiked, 
  onShowSuperLiked, 
  swipeState,
  renderCard 
}: GenericSwipeScreenProps<T>) {
  const { 
    current: currentAnimal, 
    next: nextAnimal, 
    remainingCount, 
    likedCount: likedAnimalsCount, 
    superLiked: superLikedAnimals,
    handleSwipe, 
    reset, 
    isComplete 
  } = swipeState
