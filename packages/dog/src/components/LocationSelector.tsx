...
import { japanRegions, Region, Prefecture, City, Location, getLocationDisplayName } from '@/data/locations'
...
export const LocationSelector = ({ selectedLocations = [], onLocationsChange, className = '' }: LocationSelectorProps) => {
  ...
  const handleRegionAllSelect = (region: Region) => {
    ...
    const filteredLocations = selectedLocations.filter(loc => !regionPrefectureNames.includes(loc.prefecture))
  ...
  const handleConfirmCities = () => {
    ...
    const filteredLocations = selectedLocations.filter(loc => loc.prefecture !== selectedPrefecture.name)
  ...
  const handleRemoveLocation = (location: Location) => {
    onLocationsChange(selectedLocations.filter(loc => 
      !(loc.prefecture === location.prefecture && loc.city === location.city)
    ))
  }
