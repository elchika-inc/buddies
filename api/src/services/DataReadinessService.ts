...
export class DataReadinessService {
  ...
  async evaluateAndUpdateReadiness(): Promise<{ isReady: boolean; imageCoverage: number }> {
    ...
    const imageCoverage = stats.totalPets > 0 ? stats.petsWithJpeg / stats.totalPets : 0

    const isReady =
      stats.totalDogs >= minDogs && stats.totalCats >= minCats && imageCoverage >= minCoverage
