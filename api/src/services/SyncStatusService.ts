...
export class SyncStatusService {
   ...
   * データ準備状態を更新
   */
  async updateDataReadiness(): Promise<{
    isReady: boolean
    dataCompleteness: number
    imageCoverage: number
  }> {
    // ペット統計を取得
    const petStats = await this.db
      .prepare(
        `
      SELECT 
        COUNT(*) as total,
    ...
    const dataCompleteness =
      petStats && petStats.total > 0
        ? Math.min(1.0, petStats.total / 60) // 60件で100%
        : 0

    const imageCoverage =
      imageStats && imageStats.total > 0 ? imageStats.with_jpeg / imageStats.total : 0

    // 準備完了判定（犬猫各30件以上、画像カバー率80%以上）
    const isReady = petStats && petStats.dogs >= 30 && petStats.cats >= 30 && imageCoverage >= 0.8
  ...
  async updatePetSyncStatus(
    ...
    `
      ...
      .bind(petId, hasJpeg ? 1 : 0, hasWebp ? 1 : 0, syncJobId)
      .run()
  }
