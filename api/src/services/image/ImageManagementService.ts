...
export class ImageManagementService {
  ...
  async getImageStatus(petType: 'dog' | 'cat'): Promise<ImageStatus> {
    try {
      ...
      const pets = await this.db.prepare(
        'SELECT id, name, has_jpeg, has_webp FROM pets WHERE type = ?'
      ).bind(petType).all();
  ...
  async syncImageFlags(petType: 'dog' | 'cat'): Promise<{
    ...
    try {
      // データベースからペット一覧を取得
      const pets = await this.db.prepare(
        'SELECT id, has_jpeg, has_webp FROM pets WHERE type = ?'
      ).bind(petType).all();
        ...
        try {
          ...
          if (jpegExists !== currentHasJpeg || webpExists !== currentHasWebp) {
            await this.db.prepare(`
              ...
              WHERE id = ?
            `).bind(jpegExists ? 1 : 0, webpExists ? 1 : 0, petId).run();
   ...
   * 画像の一括確認
   */
  async checkBatchImages(petIds: string[], petType: 'dog' | 'cat'): Promise<Map<string, {
    hasJpeg: boolean;
    hasWebp: boolean;
  }>> {
