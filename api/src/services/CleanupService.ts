...
export class CleanupService {
  ...
  async deleteExpiredPets(): Promise<CleanupResult> {
    ...
    try {
      ...
      const hardDeleteResult = await this.db.prepare(`
        DELETE FROM pets 
      ...
      result.errors.push(`Hard delete failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }

    try {
      const softDeleteResult = await this.db.prepare(`
        UPDATE pets 
        SET 
          is_deleted = TRUE,
      ...
      result.errors.push(`Soft delete failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  ...
  async getExpiredPetImages(): Promise<string[]> {
    try {
      const expiredPets = await this.db.prepare(`
        SELECT images 
        FROM pets 
        WHERE is_deleted = TRUE
        AND deleted_at < datetime('now', '-14 days')
