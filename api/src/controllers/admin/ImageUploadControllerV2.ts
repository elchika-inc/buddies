  ...
  async convertAndUploadImage(c: Context<{ Bindings: Env }>): Promise<Response> {
    try {
      ...
      for (const format of request.targetFormats) {
        const uploadResult = format === 'jpeg'
    ...
    try {
      ...
      for (const pet of request.results) {
        try {
          ...
          if (pet.screenshot) {
            uploadPromises.push(
              this.uploadService.uploadScreenshot(
                pet.petId,
                pet.petType,
                pet.screenshot.data,
                {
          ...
          if (allSuccessful) {
            ...
            if (pet.screenshot) {
              await this.databaseService.updateScreenshotStatus(
                pet.petId,
                pet.petType
              );
            }
      ...
      console.log(`[batch-upload] Batch ${request.batchId} complete: ${successful}/${request.results.length} successful`);
